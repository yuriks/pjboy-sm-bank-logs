...
$A0:903E 20 58 97    JSR $9758  [$A0:9758]  ; Enemy collision handler
...
$A0:96DB 8D 0D 06    STA $060D  [$7E:060D]  ;} $060D = [[Y] + 2]
$A0:96DE C8          INY                    ;\
$A0:96DF C8          INY                    ;|
$A0:96E0 C8          INY                    ;} Y += 4
$A0:96E1 C8          INY                    ;/
$A0:96E2 4A          LSR A                  ;\
$A0:96E3 90 13       BCC $13    [$96F8]     ;} If [$060D] & 1 = 0: go to BRANCH_UNROLLED

$A0:96E5 B9 00 00    LDA $0000,y[$A4:D6F4]  ;\
$A0:96E8 9F 00 00 7E STA $7E0000,x[$7E:2040];|
$A0:96EC E8          INX                    ;|
$A0:96ED E8          INX                    ;|
$A0:96EE C8          INY                    ;} Copy [$060D] words from [Y] to $7E:0000 + [X]
$A0:96EF C8          INY                    ;|
...
$A0:9709 E8          INX                    ;|
$A0:970A C8          INY                    ;|
$A0:970B C8          INY                    ;} Copy [$060D] words from [Y] to $7E:0000 + [X]
$A0:970C C8          INY                    ;|
$A0:970D C8          INY                    ;|
...
;;; $9726: Handle queuing enemy BG2 tilemap VRAM transfer ;;;
{
; Used by Crocomire and Mother Brain
...
$A0:972E 0D 5E 18    ORA $185E  [$7E:185E]  ;} If time is not frozen:
...
$A0:9772 20 5A 9A    JSR $9A5A  [$A0:9A5A]  ; Enemy / Samus collision handler - extended spritemap
$A0:9775 AB          PLB
$A0:9776 28          PLP
$A0:9777 60          RTS

$A0:9778 20 43 A1    JSR $A143  [$A0:A143]  ; Enemy / projectile collision handler
$A0:977B 20 36 A2    JSR $A236  [$A0:A236]  ; Enemy / bomb collision handler
$A0:977E 20 7A A0    JSR $A07A  [$A0:A07A]  ; Enemy / Samus collision handler
$A0:9781 AB          PLB
$A0:9782 28          PLP
$A0:9783 60          RTS
}
...
$A0:9933 BF 0A 00 86 LDA $86000A,x[$86:973E];} If [$86:000A + [enemy projectile ID]] != 0:
$A0:9937 F0 09       BEQ $09    [$9942]     ;/
$A0:9939 99 47 1B    STA $1B47,y[$7E:1B69]  ; Enemy projectile instruction pointer = [$86:000A + [enemy projectile ID]]
...
$A0:99AB C9 00 03    CMP #$0300             ;} If power bomb: go to BRANCH_NEXT_PROJECTILE
...
$A0:99FC 89 08 00    BIT #$0008             ;} If projectile is not plasma:
$A0:99FF D0 09       BNE $09    [$9A0A]     ;/
$A0:9A01 B9 04 0C    LDA $0C04,y[$7E:0C04]  ;\
$A0:9A04 09 10 00    ORA #$0010             ;} Delete projectile
$A0:9A07 99 04 0C    STA $0C04,y[$7E:0C04]  ;/

$A0:9A0A BF 80 F3 7E LDA $7EF380,x[$7E:F396];\
$A0:9A0E C9 01 00    CMP #$0001             ;} If [enemy projectile $7E:F380] != 1:
...
$A0:9A1E BF 0C 00 86 LDA $86000C,x[$86:AD6A];} Enemy projectile instruction list pointer = [$86:000C + [enemy projectile ID]] (shot instruction list)
...
;;; $9A5A: Enemy / Samus collision handler - extended spritemap ;;;
{
; This routine disables Samuss invincibility if she is using blue suit or screw attack (unless there are no tangible enemies)
$A0:9A5A 8B          PHB
...
;;; $9B7F: Enemy / projectile collision handler - extended spritemap ;;;
{
$A0:9B7F 8B          PHB
...
$A0:9B9B 8D 0F 06    STA $060F  [$7E:060F]
...
$A0:9BE9 C9 00 03    CMP #$0300             ;} If power bomb: go to BRANCH_NEXT_PROJECTILE
...
$A0:9CAC C9 00 02    CMP #$0200             ;} If super missile:
...
$A0:9CC3 89 00 10    BIT #$1000             ;|
$A0:9CC6 D0 08       BNE $08    [$9CD0]     ;} If enemy blocks plasma beam or projectile is not plasma beam:
$A0:9CC8 B9 18 0C    LDA $0C18,y[$7E:0C18]  ;|
$A0:9CCB 29 08 00    AND #$0008             ;|
...
;;; $9D23: Enemy / bomb collision handler - extended spritemap ;;;
{
...
$A0:9D89 C9 00 05    CMP #$0500             ;} If not a bomb:
...
$A0:9EA9 9C A6 18    STZ $18A6  [$7E:18A6]  ; $18A6 = 0
...
$A0:9F21 F0 21       BEQ $21    [$9F44]     ;} If enemy grapple AI != $8005:
$A0:9F23 C8          INY                    ; Y = 2
$A0:9F24 C9 0A 80    CMP #$800A             ;\
$A0:9F27 F0 1B       BEQ $1B    [$9F44]     ;} If enemy grapple AI != $800A:
$A0:9F29 C8          INY                    ; Y = 3
$A0:9F2A C9 0F 80    CMP #$800F             ;\
$A0:9F2D F0 15       BEQ $15    [$9F44]     ;} If enemy grapple AI != $800F:
$A0:9F2F C8          INY                    ; Y = 4
$A0:9F30 C9 14 80    CMP #$8014             ;\
$A0:9F33 F0 0F       BEQ $0F    [$9F44]     ;} If enemy grapple AI != $8014:
$A0:9F35 C8          INY                    ; Y = 5
$A0:9F36 C9 19 80    CMP #$8019             ;\
$A0:9F39 F0 09       BEQ $09    [$9F44]     ;} If enemy grapple AI != $8019:
$A0:9F3B C8          INY                    ; Y = 6
$A0:9F3C C9 1E 80    CMP #$801E             ;\
$A0:9F3F F0 03       BEQ $03    [$9F44]     ;} If enemy grapple AI != $801E:
...
$A0:9F70 9E 8A 0F    STZ $0F8A,x[$7E:0F8A]  ; AI handler = main AI
$A0:9F73 9E A0 0F    STZ $0FA0,x[$7E:0FA0]  ; Invincibility timer = 0
$A0:9F76 9E 9E 0F    STZ $0F9E,x[$7E:0F9E]  ; Frozen timer = 0
$A0:9F79 9E A2 0F    STZ $0FA2,x[$7E:0FA2]  ; Shake timer = 0
$A0:9F7C 6B          RTL
}
...
$A0:9FE5 9D 8A 0F    STA $0F8A,x[$7E:0F8A]
$A0:9FE8 6B          RTL
}
...
;;; $A143: Enemy / projectile collision handler ;;;
{
$A0:A143 8B          PHB
...
$A0:A181 9C A6 18    STZ $18A6  [$7E:18A6]  ; $18A6 = 0 (projectile index)

; LOOP
$A0:A184 AE 54 0E    LDX $0E54  [$7E:0E54]
$A0:A187 AD A6 18    LDA $18A6  [$7E:18A6]  ;\
$A0:A18A 0A          ASL A                  ;} Y = [$18A6] * 2
$A0:A18B A8          TAY                    ;/
$A0:A18C B9 18 0C    LDA $0C18,y[$7E:0C18]  ;\
$A0:A18F F0 12       BEQ $12    [$A1A3]     ;} If [projectile type] = 0: go to BRANCH_NEXT
$A0:A191 29 00 0F    AND #$0F00             ;\
$A0:A194 C9 00 03    CMP #$0300             ;} If (projectile type) = power bomb: go to BRANCH_NEXT
$A0:A197 F0 0A       BEQ $0A    [$A1A3]     ;/
$A0:A199 C9 00 05    CMP #$0500             ;\
$A0:A19C F0 05       BEQ $05    [$A1A3]     ;} If (projectile type) = bomb: go to BRANCH_NEXT
$A0:A19E C9 00 07    CMP #$0700             ;\
$A0:A1A1 30 03       BMI $03    [$A1A6]     ;} If (projectile type) >= 7 (explosion):
$A0:A1A3 4C 16 A2    JMP $A216  [$A0:A216]  ; Go to BRANCH_NEXT
...
$A0:A1D6 80 3E       BRA $3E    [$A216]     ; Go to BRANCH_NEXT

$A0:A1D8 B9 18 0C    LDA $0C18,y[$7E:0C18]  ;\
$A0:A1DB 29 00 0F    AND #$0F00             ;|
$A0:A1DE C9 00 02    CMP #$0200             ;} If (projectile type) = super missile:
...
$A0:A202 89 08 00    BIT #$0008             ;} If projectile is plasma: go to BRANCH_DONT_DELETE_BEAM
$A0:A205 D0 09       BNE $09    [$A210]     ;/

$A0:A207 B9 04 0C    LDA $0C04,y[$7E:0C04]  ;\
$A0:A20A 09 10 00    ORA #$0010             ;} Delete projectile
$A0:A20D 99 04 0C    STA $0C04,y[$7E:0C04]  ;/

; BRANCH_DONT_DELETE_BEAM
$A0:A210 22 26 A2 A0 JSL $A0A226[$A0:A226]  ; Execute enemy shot
$A0:A214 80 0E       BRA $0E    [$A224]     ; Return

; BRANCH_NEXT
$A0:A216 EE A6 18    INC $18A6  [$7E:18A6]  ; Increment $18A6
$A0:A219 AD A6 18    LDA $18A6  [$7E:18A6]  ;\
$A0:A21C C9 05 00    CMP #$0005             ;} If [$18A6] = 5: return
...
;;; $A236: Enemy / bomb collision handler ;;;
{
$A0:A236 8B          PHB
...
$A0:A26D 8D A6 18    STA $18A6  [$7E:18A6]  ;} $18A6 = 5 (bomb index)

; LOOP
$A0:A270 AE 54 0E    LDX $0E54  [$7E:0E54]
$A0:A273 AD A6 18    LDA $18A6  [$7E:18A6]  ;\
$A0:A276 0A          ASL A                  ;} Y = [$18A6] * 2
$A0:A277 A8          TAY                    ;/
...
$A0:A28B C9 00 05    CMP #$0500             ;} If (projectile type) != bomb:
...
$A0:A2DA 09 10 00    ORA #$0010             ;} Delete projectile
...
; BRANCH_NEXT
$A0:A2E6 EE A6 18    INC $18A6  [$7E:18A6]  ; Increment $18A6
$A0:A2E9 AD A6 18    LDA $18A6  [$7E:18A6]  ;\
$A0:A2EC C9 0A 00    CMP #$000A             ;} If [$18A6] = Ah: return
...
$A0:A37B A9 37 80    LDA #$8037             ; A = $8037
...
;;; $BB70: Calculate then block containing a pixel position ;;;
{
...
$A0:BC18 38          SEC
$A0:BC19 FD 82 0F    SBC $0F82,x[$7E:1002]
...
$A0:BC2C 30 11       BMI $11    [$BC3F]
$A0:BC2E 8A          TXA
$A0:BC2F 18          CLC
$A0:BC30 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BC33 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BC36 AA          TAX
$A0:BC37 C6 1A       DEC $1A    [$7E:001A]
$A0:BC39 10 ED       BPL $ED    [$BC28]
$A0:BC3B FA          PLX
$A0:BC3C 18          CLC
$A0:BC3D AB          PLB
$A0:BC3E 6B          RTL
...
$A0:BCE3 30 0A       BMI $0A    [$BCEF]
$A0:BCE5 E8          INX
$A0:BCE6 E8          INX
$A0:BCE7 C6 1A       DEC $1A    [$7E:001A]
$A0:BCE9 10 F4       BPL $F4    [$BCDF]
$A0:BCEB FA          PLX
$A0:BCEC 18          CLC
$A0:BCED AB          PLB
$A0:BCEE 6B          RTL
...
$A0:BDA5 30 1B       BMI $1B    [$BDC2]
$A0:BDA7 8A          TXA
$A0:BDA8 18          CLC
$A0:BDA9 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BDAC 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BDAF AA          TAX
$A0:BDB0 C6 1A       DEC $1A    [$7E:001A]
$A0:BDB2 10 ED       BPL $ED    [$BDA1]
$A0:BDB4 FA          PLX
$A0:BDB5 A5 16       LDA $16    [$7E:0016]
$A0:BDB7 9D 7C 0F    STA $0F7C,x
$A0:BDBA A5 18       LDA $18    [$7E:0018]
...
$A0:BE75 30 14       BMI $14    [$BE8B]
$A0:BE77 E8          INX
$A0:BE78 E8          INX
$A0:BE79 C6 1A       DEC $1A    [$7E:001A]
$A0:BE7B 10 F4       BPL $F4    [$BE71]
$A0:BE7D FA          PLX
$A0:BE7E A5 16       LDA $16    [$7E:0016]
$A0:BE80 9D 80 0F    STA $0F80,x
$A0:BE83 A5 18       LDA $18    [$7E:0018]
...
$A0:BF3E 30 11       BMI $11    [$BF51]
$A0:BF40 8A          TXA
$A0:BF41 18          CLC
$A0:BF42 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BF45 6D A5 07    ADC $07A5  [$7E:07A5]
$A0:BF48 AA          TAX
$A0:BF49 C6 1A       DEC $1A    [$7E:001A]
$A0:BF4B 10 ED       BPL $ED    [$BF3A]
$A0:BF4D FA          PLX
$A0:BF4E 18          CLC
$A0:BF4F AB          PLB
$A0:BF50 6B          RTL
...
;;; $BF8A:  ;;;
{
$A0:BF8A 85 1C       STA $1C    [$7E:001C]
$A0:BF8C 8B          PHB
$A0:BF8D E2 20       SEP #$20
$A0:BF8F A9 A0       LDA #$A0
$A0:BF91 48          PHA
$A0:BF92 AB          PLB
...
$A0:C009 30 0A       BMI $0A    [$C015]
$A0:C00B E8          INX
$A0:C00C E8          INX
$A0:C00D C6 1A       DEC $1A    [$7E:001A]
$A0:C00F 10 F4       BPL $F4    [$C005]
$A0:C011 FA          PLX
$A0:C012 18          CLC
$A0:C013 AB          PLB
$A0:C014 6B          RTL
...
$A0:C34E AA          TAX
$A0:C34F AC 54 0E    LDY $0E54  [$7E:0E54]
$A0:C352 A5 1C       LDA $1C    [$7E:001C]
$A0:C354 D0 15       BNE $15    [$C36B]
...
;;; $C435:  ;;;
{
; Used by enemy block collision reactions - slope - square
; Only used to check the sign and return carry set/clear accordingly
$A0:C435             db 00, 01, 82, 83, 00, 81, 02, 83, 00, 01, 02, 83, 00, 81, 82, 83, 80, 81, 82, 83
}
...
$A0:C8C6 0A          ASL A
$A0:C8C7 AA          TAX
$A0:C8C8 BF 02 00 7F LDA $7F0002,x[$7F:00EC]
...
$A0:C906 30 2B       BMI $2B    [$C933]
$A0:C908 0A          ASL A
$A0:C909 30 05       BMI $05    [$C910]
$A0:C90B B9 7A 0F    LDA $0F7A,y[$7E:0FBA]
$A0:C90E 80 06       BRA $06    [$C916]

$A0:C910 B9 7A 0F    LDA $0F7A,y[$7E:0FBA]
$A0:C913 49 0F 00    EOR #$000F

$A0:C916 29 0F 00    AND #$000F
$A0:C919 18          CLC
$A0:C91A 6D D6 0D    ADC $0DD6  [$7E:0DD6]
$A0:C91D AA          TAX
$A0:C91E BF 2B 8B 94 LDA $948B2B,x[$94:8C5A]
$A0:C922 29 1F 00    AND #$001F
$A0:C925 38          SEC
$A0:C926 ED D4 0D    SBC $0DD4  [$7E:0DD4]
$A0:C929 3A          DEC A
$A0:C92A 10 07       BPL $07    [$C933]
$A0:C92C 18          CLC
$A0:C92D 79 7E 0F    ADC $0F7E,y[$7E:117E]
...
$A0:C946 0A          ASL A
$A0:C947 AA          TAX
$A0:C948 BF 02 00 7F LDA $7F0002,x[$7F:00EC]
...
$A0:C988 10 31       BPL $31    [$C9BB]
$A0:C98A 0A          ASL A

$A0:C98B 30 05       BMI $05    [$C992]
$A0:C98D B9 7A 0F    LDA $0F7A,y[$7E:0F7A]
$A0:C990 80 06       BRA $06    [$C998]

$A0:C992 B9 7A 0F    LDA $0F7A,y[$7E:0F7A]
$A0:C995 49 0F 00    EOR #$000F

$A0:C998 29 0F 00    AND #$000F
$A0:C99B 18          CLC
$A0:C99C 6D D6 0D    ADC $0DD6  [$7E:0DD6]
$A0:C99F AA          TAX
$A0:C9A0 BF 2B 8B 94 LDA $948B2B,x[$94:8C4B]
$A0:C9A4 29 1F 00    AND #$001F
$A0:C9A7 38          SEC
$A0:C9A8 ED D4 0D    SBC $0DD4  [$7E:0DD4]
$A0:C9AB 3A          DEC A
$A0:C9AC F0 02       BEQ $02    [$C9B0]
$A0:C9AE 10 0B       BPL $0B    [$C9BB]
...
$A0:CF7F             dx 0C00, 8B60, 4E20, 0000, 0008, 0005, A2, 00, 0000, 0000, 8D9D, 0001, 0000, 912E, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 929F, 930F, 0000, ACD400, 05, F3BC, EEC6, 0000
$A0:CFBF             dx 0200, 998D, 0064, 003C, 0008, 0004, A2, 00, 0000, 0000, 9A3F, 0001, 0000, 9A7D, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, ACE400, 05, F20C, EC1C, E053
$A0:CFFF             dx 0400, 9E6A, 003C, 0014, 0008, 0008, A2, 00, 0000, 0000, 9F48, 0001, 0000, 9FB3, 800A, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, ACE600, 05, F2BA, EC1C, E0C3
$A0:D03F             dx 0400, A38B, 0014, 0064, 0008, 0008, A2, 00, 003E, 0000, A3F9, 0001, 0000, A47E, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, A579, 0000, ACEA00, 05, F34A, EE84, E16B
...
$A0:D0FF             dx 0400, AFF3, 0009, 0008, 0008, 0004, A2, 00, 0020, 0000, B06B, 0001, 0000, B11F, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, B013, 00000000, 8023, 802D, B204, AEA600, 05, F236, EC1C, DF81
$A0:D13F             dx 0400, B20C, 001E, 0010, 0008, 0004, A2, 00, 0020, 0000, B06B, 0001, 0000, B11F, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, B22C, 00000000, 8023, 802D, B25C, AEC920, 05, F23C, EE00, DF65
$A0:D17F             dx 0400, B264, 0064, 003C, 0008, 0008, A2, 00, 0020, 0000, B06B, 0001, 0000, B11F, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, B284, 00000000, 8023, 802D, B2B4, AECD20, 05, F242, EC1C, DF73
$A0:D1BF             dx 0400, B2BC, 005A, 0032, 0008, 0008, A2, 00, 003E, 0000, B3E0, 0001, 0000, B40F, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, B2DC, 00000000, 8023, 802D, 0000, AEB400, 05, F362, EE6E, DFB9
$A0:D1FF             dx 0600, BA5B, 0001, 0004, 0004, 0004, A2, 00, 0000, 0000, B570, 0001, 0000, B58F, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEB800, 05, F48E, EEC6, E029
$A0:D23F             dx 0600, BA5B, 000A, 0028, 0008, 0008, A2, 00, 0000, 0000, B602, 0001, 0000, B7C4, 800F, 804C, B929, 0000, 0000, 00000000, B953, 0000, 00000000, B947, B94D, 0000, AEB800, 02, F374, EC1C, E06F
$A0:D27F             dx 0400, BA7B, 002D, 000F, 0010, 0007, A2, 00, 0036, 0000, BBCD, 0001, 0000, BBE3, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEB000, 05, F1FA, EC1C, E07D
$A0:D2BF             dx 0400, BE1C, 012C, 0032, 0008, 000C, A2, 00, 0036, 0000, BE99, 0002, 0000, BED2, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEBD20, 05, F35C, EDEA, E141
$A0:D2FF             dx 0600, C0D1, 0078, 003C, 0010, 0009, A2, 00, 0036, 0000, C242, 0002, 0000, C277, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AED520, 05, F1F4, EE42, DE85
$A0:D33F             dx 0800, C5FA, 0384, 0078, 0010, 000A, A2, 00, 005F, 0000, C6F3, 0002, 0000, C724, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AED920, 05, F356, EE58, DED9
$A0:D37F             dx 0800, CA2B, 012C, 0064, 0010, 0010, A2, 00, 0000, 0000, CCD4, 0001, 0000, CD13, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, D38C, D3B4, 0000, AF8000, 05, F338, EEC6, DFFF
$A0:D3BF             dx 0400, D80C, 0064, 0050, 0010, 0005, A2, 00, 0022, 0000, DF76, 0001, 0000, E02E, 800A, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, ACE000, 05, F21E, EC1C, DECB
$A0:D3FF             dx 0400, E17B, 00C8, 000A, 0008, 0008, A2, 00, 003E, 0000, E1D3, 0001, 0000, E221, 8005, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, E3A9, 0000, AEC520, 05, F2C6, EF4A, E099
$A0:D43F             dx 0400, E2C0, 00C8, 000A, 0008, 0004, A2, 00, 003E, 0000, E318, 0001, 0000, E353, 800F, 804C, 8041, 0000, 0000, 00000000, 0000
 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AFE800, 05, F260, EC1C, E1CD
$A0:DABF             dx 0400, B5B3, 001E, 000F, 0008, 0008, A3, 00, 005F, 0000, B66F, 0001, 0000, E6C2, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, B5D3, 00000000, 8023, 802D, 0000, AEF920, 05, F2CC, EC48, E179
$A0:DAFF             dx 0000, 8069, 0000, 0000, 0008, 0008, A3, 00, 0059, 0000, 804C, 0001, 0000, 804C, 800F, 804C, 804C, 0000, 0000, 00000000, 0000, 0000, 00000000, 804C, 804C, 0000, AE9000, 00, 0000, 0000, 0000
$A0:DB3F             dx 1000, B702, 000A, 0004, 0010, 0010, A3, 00, 0056, 0000, BAB3, 0003, 0000, BB25, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 8023, BEFD, 0000, AF9800, 05, F380, F010, DDA5
$A0:DB7F             dx 0400, C63E, 000F, 000A, 0008, 000C, A3, 00, 0021, 0000, C6AE, 0001, 0000, C6C7, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, C7F5, 0000, AE8C00, 05, F206, EC1C, E125
$A0:DBBF             dx 0800, C8A6, 000A, 0064, 0008, 0008, A3, 00, 003E, 0000, CDE2, 0001, 0000, CE64, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, D3B0, D469, 0000, AFB400, 05, F33E, EC48, E195
$A0:DBFF             dx 0600, DA9C, 00C8, 001E, 0010, 0010, A3, 00, 0057, 0000, DBD3, 0001, 0000, DC1B, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 804C, DC1C, 0000, AFAE00, 05, F31A, EEC6, E061
$A0:DC3F             dx 0600, DFA2, 000F, 0005, 0008, 0008, A3, 00, 0023, 0000, E043, 0001, 0000, E08B, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AE8000, 05, F2FC, EC48, DEF5
$A0:DC7F             dx 0600, E23C, 001E, 000A, 0008, 0008, A3, 00, 0023, 0000, E2D4, 0001, 0000, E6C2, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEED20, 05, F22A, EC48, E1BF
$A0:DCBF             dx 0600, E57C, 0028, 0014, 0008, 0008, A3, 00, 0023, 0000, E59C, 0001, 0000, E6C2, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEF320, 05, F230, EC48, DFF1
$A0:DCFF             dx 0600, E5B0, 000F, 0005, 0008, 0008, A3, 00, 0023, 0000, E669, 0001, 0001, E6C2, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AE8000, 05, F224, EC48, E1DB
$A0:DD3F             dx 0600, E5D0, 000F, 0005, 0008, 0008, A3, 00, 0023, 0000, E669, 0001, 0001, E6C2, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AE8000, 05, F302, EDD4, DFC7
$A0:DD7F             dx 1000, E9AF, 01F4, 0078, 000A, 000A, A3, 00, 005A, 0000, EA4F, 0001, 0000, EB98, 800F, EB33, EAE6, 0000, 0004, 00000000, F042, 0000, 00000000, EDEB, EF07, 0000, AE9000, 05, F36E, EF60, DFAB
$A0:DDBF             dx A600, B87D, 7FFF, 0028, 0030, 0038, A4, 00, 0005, 0006, 8A5A, 0001, 0000, 8C04, 8005, 8687, 8041, 0000, 0000, 00000000, B992, 0000, 00000000, B950, 0000, 0000, AD8000, 05, F42E, F102, 0000
$A0:DDFF             dx 2000, B87D, 7FFF, 0028, 0008, 0008, A4, 00, 0000, 0006, F67A, 0001, 0000, F6BB, 8005, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AD8000, 05, F428, F102, 0000
$A0:DE3F             dx 2000, A1F7, 1770, 00A0, 0040, 0040, A5, 04, 0000, 0008, 8687, 0001, 0000, 86FC, 8000, 954D, 8041, 0000, 0000, 00000000, 9607, 0000, 00000000, 95EA, 95F0, 0000, B0C800, 05, F440, F144, 0000
$A0:DE7F             dx 1800, 8069, 1770, 00A0, 0002, 0002, A5, 00, 0000, 0000, C46B, 0001, 0000, C486, 800F, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, B0C800, 05, F446, F144, 0000
$A0:DEBF             dx 1800, A1F7, 1770, 00A0, 0010, 0020, A5, 00, 0000, 0000, C599, 0001, 0000, C5AA, 800F, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 8023, 802D, 0000, B0C800, 05, F440, F144, 0000
$A0:DEFF             dx 1800, A1F7, 1770, 00A0, 0018, 0018, A5, 00, 0000, 0000, C5AD, 0001, 0000, C5C4, 800F, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, B0C800, 05, F440, F144, 0000
$A0:DF3F             dx 0E00, E359, 03C0, 000C, 0030, 0020, A5, 04, 002C, 0004, EA2A, 0001, 0000, EB13, 800F, 804C, 8041, 0000, 0000, 00000000, EDF2, 0000, 00000000, EDEC, ED5A, 0000, AC9C00, 02, F410, F094, 0000
$A0:DF7F             dx 0E00, E359, 03C0, 000C, 0013, 0013, A5, 08, 0000, 0004, EA2A, 0001, 0000, EB13, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, EDEC, ED5A, 0000, AC9C00, 05, F416, F094, 0000
$A0:DFBF             dx 1000, 8687, 0014, 0028, 0010, 0010, A6, 00, 0000, 0000, 86F5, 0001, 0000, 8793, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AB9C00, 05, F46A, EEDC, E0B5
$A0:DFFF             dx 0400, 8B09, 01F4, 00C8, 0010, 000C, A6, 00, 0000, 0000, 8B2F, 0002, 0000, 8BAD, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 804C, 0000, AB9800, 05, F470, EEC6, DF49
$A0:E03F             dx 0200, 8B09, 01F4, 00C8, 0010, 0002, A6, 00, 0000, 0000, 8B85, 0001, 0000, 8B99, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 804C, 0000, AB9800, 05, F470, EEC6, 0000
$A0:E07F             dx 0800, 8CFB, 0014, 001E, 0008, 0008, A6, 00, 0000, 0000, 8FFC, 0002, 0000, 9023, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 804C, 0000, AB9000, 05, F476, EEC6, DEBD
$A0:E0BF             dx 0800, 9470, 0028, 0032, 0008, 0008, A6, 00, 0000, 0000, 94C4, 0001, 0000, 960E, 8000, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 804C, 0000, AB9000, 05, F464, EEC6, E037
$A0:E0FF             dx 1000, 998C, 0190, 0064, 0020, 0018, A6, 00, 0000, 0000, 9A58, 0001, 0000, 9AC2, 800F, 804C, 8041, 0000, 0003, 00000000, 9C39, 0000, 00000000, 9C22, 9C39, 0000, AB8000, 05, F2C0, EFFA, E0DF
...
$A0:E1BF             dx 8400, E1AF, 4650, 00A0, 0008, 0008, A6, 00, 0000, 0000, C696, 0001, 0000, C8D4, 800F, 804C, 804C, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, B0B400, 05, 0000, EEC6, 0000
$A0:E1FF             dx 0000, E23F, 7FFF, 0000, 0008, 0008, A6, 00, 0000, 0000, EFB1, 0001, 0000, F00D, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, F03F, 804C, 0000, A0E23F, 05, F488, EEC6, 0000
$A0:E23F             dx 8600, F4EC, 0028, 000F, 0008, 0020, A6, 00, 0059, 0000, F6C5, 0001, 0000, F765, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, F52C, 00000000, F920, F920, FB72, B0BA00, 02, 0000, 0000, 0000
$A0:E27F             dx 0000, 0000, 03E8, 0000, 0008, 0018, A6, 00, 0000, 0000, FB72, 0001, 0000, FC33, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, FDA7, FDAC, 0000, B78000, 05, 0000, F068, 0000
$A0:E2BF             dx 1E00, 8687, 03E8, 0014, 0038, 0090, A7, 00, 0000, 0003, A959, 0001, 0000, AC21, 800F, 804C, 8041, 0000, 0000, 00000000, 803C, 0000, 00000000, 949F, 804C, 0000, ABCC00, 05, F41C, F15A, 0000
$A0:E2FF             dx 1E00, 8687, 03E8, 0014, 0030, 0030, A7, 00, 0000, 0000, AB43, 0001, 0000, B7BD, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 9490, 94B5, 0000, ABCC00, 05, 0000, F170, 0000
$A0:E33F             dx 1E00, 8687, 03E8, 000A, 0018, 0008, A7, 00, 0000, 0000, AB68, 0001, 0000, B801, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 948F, 802D, 0000, ABCC00, 05, 0000, F186, 0000
$A0:E37F             dx 1E00, 8687, 03E8, 000A, 0018, 0008, A7, 00, 0000, 0000, AB9C, 0001, 0000, B80D, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 948F, 802D, 0000, ABCC00, 05, 0000, F186, 0000
$A0:E3BF             dx 1E00, 8687, 03E8, 000A, 0018, 0008, A7, 00, 0000, 0000, ABCA, 0001, 0000, B819, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 948F, 802D, 0000, ABCC00, 05, 0000, F186, 0000
$A0:E3FF             dx 1E00, 8687, 03E8, 0014, 0008, 0008, A7, 00, 0000, 0000, ABF8, 0001, 0000, B9F6, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 948F, 94B5, 0000, ABCC00, 05, F422, F170, 0000
$A0:E43F             dx 1E00, 8687, 000A, 000A, 0008, 0008, A7, 00, 0000, 0000, BCEF, 0001, 0000, BD32, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, BCCF, 802D, 0000, ABCC00, 05, F422, F19C, 0000
$A0:E47F             dx 1E00, 8687, 000A, 000A, 0008, 0008, A7, 00, 0000, 0000, BD2D, 0001, 0000, BD49, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, BCDE, 802D, 0000, ABCC00, 05, 0000, F19C, 0000
$A0:E4BF             dx 0C00, CA01, 09C4, 0028, 0008, 0008, A7, 08, 0000, 0007, CDF3, 0001, 0000, CEA6, 8000, DD3F, 804C, 0000, 0000, 00000000, DD9A, 0000, 00000000, DD95, DD9B, 0000, ACAA00, 05, F434, F0EC, 0000
$A0:E4FF             dx 0400, CA01, 09C4, 0028, 0008, 0008, A7, 00, 0000, 0000, CE55, 0001, 0000, 804C, 8000, 804C, 804C, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, ACAA00, 05, F43A, F0EC, 0000
 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, BE2E, BEAC, 0000, B1C200, 05, F2F6, EF8C, DFE3
$A0:E8BF             dx 0400, C143, 000A, 0064, 0008, 0010, A8, 00, 006A, 0000, C1C9, 0002, 0000, C21C, 8014, 804C, 8041, 0000, 0002, 00000000, C63F, 0000, 00000000, C5BE, C5EF, 0000, B1CC00, 05, F2B4, EF34, E045
$A0:E8FF             dx 0C00, C6B3, 0320, 0050, 000C, 0020, A8, 00, 0068, 0000, CB77, 0001, 0000, CC36, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, D174, D192, 0000, B1D000, 05, F3EC, EEC6, E0A7
$A0:E93F             dx 0C00, C6B3, 01C2, 0050, 000C, 0020, A8, 00, 0068, 0000, CBCC, 0001, 0000, CC66, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, D174, D18D, 0000, B1D000, 05, F3F2, EEC6, E2E5
$A0:E97F             dx 0600, D821, 0064, 000A, 0008, 0008, A8, 00, 0000, 0000, D8C9, 0001, 0000, D90B, 800A, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, DB14, 0000, B1DC00, 05, F2D2, EFA2, E01B
$A0:E9BF             dx 0800, DBC7, 00C8, 0032, 0008, 0018, A8, 00, 005F, 0000, DCCD, 0001, 0000, DD6B, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, B1E200, 05, F2EA, EE2C, DFD5
$A0:E9FF             dx 0400, E230, 00FA, 0028, 0008, 0008, A8, 00, 003E, 0000, E388, 0001, 0000, E3C3, 800F, 804C, 8041, 0000, 0002, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, B1FC00, 05, F212, EC1C, DD97
...
$A0:EA7F             dx 0200, E7AC, 0014, 0000, 0008, 0008, A8, 00, 0000, 0000, E82E, 0001, 0000, E8AE, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 804C, E91D, 0000, B1BA00, 02, F45E, EEC6, DF3B
$A0:EABF             dx 1000, E99A, 003C, 0014, 0008, 0012, A8, 00, 0066, 0000, F188, 0002, 0000, F25C, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 8023, F701, 0000, B1EC00, 05, F272, ED66, DEA1
$A0:EAFF             dx 0200, E99A, 003C, 0014, 000A, 000A, A8, 00, 0066, 0000, F214, 0001, 0000, F262, 800F, 804C, 8041, 0000, 0004, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, B1EC00, 05, F272, ED66, 0000
$A0:EB3F             dx 1000, E9BA, 0168, 003C, 0008, 0012, A8, 00, 0066, 0000, F188, 0002, 0000, F25C, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 8023, F701, 0000, B1EC00, 05, F278, ED7C, E2C9
$A0:EB7F             dx 0200, E9BA, 0168, 003C, 000A, 000A, A8, 00, 0066, 0000, F214, 0001, 0000, F262, 800F, 804C, 8041, 0000, 0004, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, B1EC00, 05, F278, ED7C, 0000
$A0:EEBF             dx 0C00, F8E6, 0C80, 0028, 0028, 0028, A9, 00, 0000, 0000, EF37, 0001, 0000, EFC5, 800F, 804C, 8041, 0000, 0000, 00000000, EFBA, 0000, 00000000, F789, F842, 0000, B18400, 02, 0000, F12E, 0000
$A0:EEFF             dx 2000, 8687, 0320, 0008, 0012, 0030, AA, 08, 0027, 0002, C87F, 0001, 0000, C6A4, 800F, C67E, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, C977, C97C, 0000, AFC200, 05, F3F8, F0C0, 0000
$A0:EF3F             dx 2000, 8687, 0320, 0008, 0012, 0030, AA, 08, 0027, 0002, C87F, 0001, 0000, C6A4, 800F, C67E, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, C977, C97C, 0000, AFC200, 05, F3FE, F0C0, 0000
$A0:EF7F             dx 2000, 8687, 34BC, 00A0, 0012, 0030, AA, 08, 0027, 0002, C87F, 0001, 0000, D369, 800F, D3BA, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, C977, D667, 0000, AFC200, 05, F404, F0D6, 0000
$A0:EFBF             dx 2000, 8687, 34BC, 00A0, 0012, 0030, AA, 08, 0027, 0002, C87F, 0001, 0000, D369, 800F, D3BA, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, C977, D667, 0000, AFC200, 05, F40A, F0D6, 0000
$A0:EFFF             dx 1600, D745, 0BB8, 0BB8, 0000, 0000, AA, 00, 0000, 0000, D7C8, 0001, 0000, D7C7, 800F, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, 87B364, 06, 0000, EC1C, 0000
$A0:F03F             dx 8600, D745, 0BB8, 0BB8, 0000, 0000, AA, 00, 0000, 0000, 804C, 0001, 0000, 804C, 800F, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, 87AD64, 06, 0000, EC1C, 0000
$A0:F07F             dx 0400, D911, 012C, 0078, 0006, 0006, AA, 08, 0069, 0000, DE43, 0007, 0000, DCA3, 800F, DCA3, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, DF2F, DF34, 0000, B19000, 05, F3C8, EF1E, DE07
$A0:F0BF             dx 1800, E2BD, 0014, 0028, 0000, 0000, AA, 00, 0000, 0000, E716, 0001, 0000, 804C, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 804C, 804C, 0000, ABB400, 05, F452, EC1C, 0000
$A0:F0FF             dx 1200, E31D, 0014, 0028, 0010, 0024, AA, 00, 0000, 0000, E725, 0001, 0000, E7A7, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, E7DB, E7DC, 0000, ACBE00, 05, F452, EEC6, 0000

; Random bunch of 02's (20 of them to be exact)
$A0:F13F             db 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02, 02

$A0:F153             dx 0200, 8687, 0014, 0028, 0008, 0008, B3, 00, 0000, 0000, 86FB, 0001, 0000, 870E, 800F, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, B7FB00, 05, 0000, 0000, 0000
$A0:F193             dx 0400, 878B, 0009, 0006, 0008, 0008, B3, 00, 0022, 0000, 883B, 0001, 0000, 887A, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEC120, 05, F248, EC1C, E1A3
$A0:F1D3             dx 0400, 89FD, 001E, 0014, 0008, 0008, B3, 00, 0022, 0000, 883B, 0001, 0000, 887A, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEE520, 05, F254, EC1C, E1B1
$A0:F213             dx 0400, 8AC1, 0014, 0010, 0008, 0008, B3, 00, 0022, 0000, 8B61, 0005, 0000, 8B9E, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEE920, 05, F25A, EC1C, DE69
$A0:F253             dx 0400, 8EDC, 000A, 000A, 0008, 0008, B3, 00, 0022, 0000, 8F4C, 0001, 0000, 8FAE, 800A, 804C, 8041, 0000, 0000, 00000000, 0000, 0000, 00000000, 8023, 802D, 0000, AEE120, 05, F24E, EC1C, DE77
$A0:F293             dx 1800, 9319, 0BB8, 0078, 0008, 0008, B3, 00, 005F, 0009, 9583, 0001, 0000, 9668, 800F, 804C, 8041, 0000, 0000, 00000000, A041, 0000, 00000000, 9FFF, A016, 0000, B7E300, 05, F344, F118, DDDD
$A0:F2D3             dx 0600, E525, 0BB8, 0BB8, 0006, 0008, B3, 00, 0000, 0000, E6CB, 0001, 0000, E655, 8000, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, AC8200, 05, F482, 0000, 0000
$A0:F313             dx 0C00, E944, 0BB8, 0BB8, 0008, 0018, B3, 00, 0000, 0000, EAE5, 0001, 0000, EB1A, 8000, 804C, 8041, 0000, 0000, 00000000, 804C, 0000, 00000000, 804C, 804C, 0000, AC8800, 05, F488, 0000, 0000
$A0:F353             dx 0C00, 8687, 0014, 000F, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F2A8, ECCC, DDB3
$A0:F393             dx 0C00, 86A7, 005A, 0014, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F2AE, ECCC, E1E9
$A0:F3D3             dx 0C00, 86E7, 00C8, 0050, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F290, ECB6, E1F7
$A0:F413             dx 0C00, 8727, 0384, 00C8, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F2A2, ED50, E205
$A0:F453             dx 0C00, 86C7, 012C, 00A0, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F3A4, ECE2, E213
$A0:F493             dx 0C00, 8707, 01F4, 000F, 0010, 0018, B2, 00, 0021, 0000, EF9F, 0001, 0000, F02D, 800F, 804C, 8041, 0000, 0004, 00000000, 0000, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F4A6, EC8A, E221
$A0:F4D3             dx 1800, 8687, 0014, 000F, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0000, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F386, EC5E, DDC1
$A0:F513             dx 1800, 86A7, 005A, 0014, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0000, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F392, EC5E, E22F
$A0:F553             dx 1800, 86E7, 00C8, 0050, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0001, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F296, EC5E, E23D
$A0:F593             dx 1800, 8707, 0708, 0064, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0000, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F284, EC74, E24B
$A0:F5D3             dx 1800, 86C7, 012C, 00A0, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0000, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F3AA, EC5E, E259
$A0:F613             dx 1800, 8707, 01F4, 000F, 0010, 0020, B2, 00, 0021, 0000, F5DE, 0001, 0000, F6A2, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F4AC, ECA0, E267
$A0:F653             dx 0C00, 8687, 0014, 000F, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F38C, ECF8, DDCF
$A0:F693             dx 0C00, 86A7, 005A, 0014, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F398, ED0E, E275
$A0:F6D3             dx 0C00, 86E7, 00C8, 0050, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F29C, ED24, E283
$A0:F713             dx 0C00, 8727, 0384, 00C8, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F39E, ED50, E291
$A0:F753             dx 0C00, 86C7, 012C, 00A0, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F3B0, ED3A, E29F
$A0:F793             dx 0C00, 8707, 01F4, 000F, 0010, 0020, B2, 00, 0066, 0000, FD02, 0001, 0000, FD32, 800F, 804C, 8041, 0000, 0004, 00000000, 8767, 0000, 00000000, 876C, 8779, 0000, ADC600, 05, F4B2, EC8A, E2AD
}
